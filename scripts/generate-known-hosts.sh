...
if [ "${cmd_rc}" = "0" ]; then
  mail01_ext_key="$(
    ssh -o BatchMode=yes "${KO_ADMIN_USER}@${KO_MAIL_01_EXT_PUBLIC_ENTRY}" \
      'cat /etc/ssh/ssh_host_ed25519_key.pub' 2>/dev/null | extract_ed25519_key
  )"
  if [ "$?" = "0" ]; then
    add_known_host_line "${KO_PROJECT_PREFIX}-mail-01-ext,${KO_MAIL_01_EXT_PUBLIC_ENTRY}" "${mail01_ext_key}"
    printf 'OK | fetched %s-mail-01-ext host key over public SSH path\n' "${KO_PROJECT_PREFIX}" | mask_private
  else
    printf 'FAIL | could not fetch mail-01-ext host key over public SSH path\n' | mask_private
    cmd_rc=1
  fi
fi
...
if [ "${cmd_rc}" = "0" ]; then
  ...
  if [ "${line_count}" = "7" ]; then
    printf 'OK | candidate contains 7 host key lines\n'
  else
    printf 'FAIL | candidate contains %s lines, expected 7\n' "${line_count}"
    cmd_rc=1
  fi
fi
